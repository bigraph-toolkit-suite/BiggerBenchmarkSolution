...
//            Console.WriteLine("rewritingSeq=" + rewritingSeq);
//            procEnv = new LGSPGraphProcessingEnvironment(graph, actions);
...
//            Environment.Exit(0);
            ...
            while (workQueue.Count > 0)
            {
                ...
                foreach (string ruleName in allRules)
                {
                    ...
                    // (Variant B)
                    // IMatches matches = currentRule.Match(_procEnv, -1, null); // Find all matches
                    // // IMatches matches = _procEnv.Match(actions.GetAction(ruleName), new object[] {}, -1, false, new List<FilterCall>(), true);
                    // Console.WriteLine("Matches Found (Rule = " + ruleName + "): " + matches.Count);
                    // foreach (IMatch match in matches)
                    // {
                    //     currentRule.Modify(_procEnv, match); // Apply the match to new graph state
                    //     // currentRule.Apply(_procEnv); // Apply the match to new graph state
                    //     LGSPNamedGraph newBigraph = (LGSPNamedGraph)_procEnv.graph.CloneAndAssignNames();
                    //     // LGSPNamedGraph newBigraph = (LGSPNamedGraph)_procEnv.graph.Clone("state-" + gIx);
                    //     reactionResults.Add(new MatchResult(newBigraph, ruleName, match, newBigraph.Canonize()));
                    // }

                    // (Variant C)
                    // IMatches matchesInexact;
                    // while ((matchesInexact = currentRule.Match(_procEnv, 1, null)).Count == 1)
                    // {
                    //    Console.WriteLine("Match found for rule " + ruleName);
                    //    currentRule.Modify(_procEnv, matchesInexact.First);
                    //    LGSPNamedGraph newBigraph = (LGSPNamedGraph)_procEnv.graph.CloneAndAssignNames();
                    //    reactionResults.Add(new MatchResult(newBigraph, ruleName, matchesInexact.First, newBigraph.Canonize()));
                    // }
                }
